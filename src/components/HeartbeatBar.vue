...
export default {
    ...
    computed: {
        ...
        /**
         * If heartbeatList is null, use the full per-monitor history when
         * available and fall back to the dashboard slice.
         * @returns {object} Heartbeat list
         */
        beatList() {
            if (this.heartbeatList === null) {
                return this.$root.heartbeatList[this.monitorId]
                    || this.$root.dashboardHeartbeatList[this.monitorId]
                    || [];
            } else {
    ...
    beforeMount() {
        if (this.heartbeatList === null) {
            if (!(this.monitorId in this.$root.dashboardHeartbeatList)) {
                this.$root.dashboardHeartbeatList[this.monitorId] = [];
            }
        }
    },
    ...
    methods: {
        ...
        getBeatTitle(beat) {
            ...
            return `${this.$root.datetime(beat.time)}` + (msg ? ` - ${msg}` : "");
        },
